...
var View = Event.extend({
	...
	*/
	init: function (opts) {
		...
		if (!this.template && opts.template)
	...
	*/
		...
		function handleSingleChange (evt) {
			...
			if (value !== undefined) {
				...
				if (this.model[key] !== value) {
					this.emit("prechange", key, value, input);
					this.emit("prechange:"+key, value, input);

					this.model[key] = value;
					
					this.emit("change", key, value, oldvalue, input);
					this.emit("change:"+key, value, oldvalue, input);
				}
			}
		}
	...
	reset: function () {
		for (var key in this.model) {
			this.model[key] = this.defaults[key];
		}
	...
	*/
	...
	set: function (key, value) {
		...
		//check if the key is in the forms
		for (var i = 0; i < this.form.length; ++i) {
			var radio = this.form[i];
			if (radio.type === "radio" && radio.name == key) {
...
Spineless.Message = new Event();
Spineless.setText = function (el, text) {
